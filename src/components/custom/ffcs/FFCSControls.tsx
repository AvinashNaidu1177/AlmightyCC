...
export default function FFCSControls({ courses, setCourses }: { courses: Course[], setCourses: React.Dispatch<React.SetStateAction<Course[]>> }) {
    ...
    const handleSelectSlot = (combo: string[]) => {
        ...
        if (exists) {
            ...
            newCombos = newCombos.filter(c => (c[0] && c[0].startsWith("L")) !== isLab);
    ...
    const autoPopulate = () => {
        try {
            ...
            if (!attRaw) {
                alert("No attendance data found to auto-populate.");
                return;
            }
            ...
            if (addedCount > 0) {
                ...
                alert(`Auto-populated ${addedCount} courses from your VTOP data!`);
            } else {
                alert("No new courses found to add.");
            }
        } catch (e) {
            console.error(e);
            alert("Failed to parse attendance data for auto-population.");
        }
    };
    ...
    const handleAddCourse = () => {
        ...
        if (savedCourse && savedCourse.hasConflict) {
            alert(`Warning: ${newCourse.code} added, but there is a time clash!`);
        } else {
            alert(`${newCourse.code} ${editingId ? "updated" : "added"}.`);
        }
    };
                ...
                {groups.map((combo, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-mono rounded-full border transition-colors ${isSelected ? "bg-purple-600 border-purple-600 text-white shadow-md shadow-purple-500/30" : "bg-white dark:bg-[#1f2937] border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-purple-400"}`}
                        >
                            {combo.join(" + ")}
                        </button>
                    );
            ...
            <div className="border border-gray-200 dark:border-gray-800 rounded-lg p-4 bg-gray-50 dark:bg-gray-900/50">
                <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-200 dark:border-gray-800">
                    <div className="flex bg-gray-200 dark:bg-gray-800 p-1 rounded-md">
                        <button onClick={() => setCurrentPane("theory")} className={`px-4 py-1.5 text-xs font-bold rounded ${currentPane === "theory" ? "bg-white dark:bg-gray-700 text-purple-600 dark:text-purple-400 shadow-sm" : "text-gray-500"}`}>THEORY</button>
                        <button onClick={() => setCurrentPane("lab")} className={`px-4 py-1.5 text-xs font-bold rounded ${currentPane === "lab" ? "bg-white dark:bg-gray-700 text-pink-600 dark:text-pink-400 shadow-sm" : "text-gray-500"}`}>LAB</button>
                    </div>
                    {selectedCombos.length > 0 && (
                        <div className="text-sm font-mono text-gray-700 dark:text-gray-300">
                            &gt; <span className="text-purple-600 dark:text-purple-400 font-bold">{Array.from(new Set(selectedCombos.flat())).join(" + ")}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-8 border-t border-gray-200 dark:border-gray-800 pt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {recalcConflicts(courses).map(c => (
                            <div key={c.id} className={`p-3 rounded-lg border ${c.hasConflict ? "border-red-500 bg-red-50 dark:bg-red-900/10" : "border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900"} flex flex-col gap-1`}>
                                <div className="flex justify-between items-start">
                                    <div className={`font-bold font-mono ${c.hasConflict ? "text-red-600 dark:text-red-400" : "text-purple-600 dark:text-purple-400"}`}>{c.code}</div>
                                    <div className="flex gap-1">
                                        <button onClick={() => editCourse(c)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded text-gray-500"><Edit2 className="w-3 h-3" /></button>
                                ...
                                <div className="text-xs text-gray-600 dark:text-gray-400 font-mono">{c.slots.join("+")}</div>
