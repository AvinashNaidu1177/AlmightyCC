...
export default function FFCSTimetableGrid({ courses }: { courses: Course[] }) {
 ...
 return (
 <div className="overflow-x-auto w-full border border-gray-800 rounded-lg pb-4">
 <table className="w-full text-xs font-mono border-collapse min-w-[800px]">
 <thead>
 <tr className="bg-gray-900 border-b border-gray-800">
 <th className="p-2 border-r border-gray-800 w-16" rowSpan={3}>DAY</th>
 <th className="p-2 border-r border-gray-800 w-16" rowSpan={3}>
 <span className="text-purple-400 font-bold block text-[10px]">Theory</span>
 <span className="text-gray-500 block text-[10px] my-1">─────</span>
 <span className="text-pink-400 font-bold block text-[10px]">Lab</span>
 </th>
 {/* Theory Time Headers */}
 {THEORY_TIMES.slice(0, 6).map((t, i) => (
 <th key={i} className="p-1 border-r border-gray-800 font-normal text-[10px] text-gray-400">
 {t}<br />-{THEORY_TIMES_END[i]}
 </th>
 ))}
 <th className="p-1 border-r border-gray-800 text-[10px] w-8 bg-gray-800" rowSpan={3}>🍽<br />LUNCH</th>
 ...
 <tr className="bg-gray-900 border-b border-gray-800">
 {[...Array(6)].map((_, i) => (
 <th key={`p${i}`} className="p-1 border-r border-gray-800 font-bold text-[10px] text-gray-300">P{i + 1}</th>
 ))}
 {[...Array(6)].map((_, i) => (
 <th key={`p${i + 6}`} className="p-1 border-r border-gray-800 font-bold text-[10px] text-gray-300">P{i + 7}</th>
 ))}
 </tr>
 </thead>
 <tbody>
 ...
 <td className="p-2 font-bold text-center border-r border-gray-800" rowSpan={2}>{day}</td>
 <td className="p-1 text-[10px] font-bold text-center text-purple-400 border-r border-gray-800">Theory</td>
 {tBlocks.map((block, raw) => {
 if (block === "LUNCH") return <td key={raw} className="border-r border-gray-800 bg-gray-800" rowSpan={2}></td>;
 if (block === "-") return <td key={raw} className="text-center p-1 border-r border-gray-800 text-gray-400">—</td>;
 ...
 if (dupConf || timeOver) bgClass = "bg-red-100 dark:bg-red-900/40 border-red-500 text-red-400 font-bold";
 ...
 return (
 <td key={raw} className={`p-1 text-center text-[10px] border border-gray-800 ${bgClass}`} title={timeOver ? "Theory + Lab clash!" : ""}>
 ...
 <tr className="border-b border-gray-800">
 <td className="p-1 text-[10px] font-bold text-center text-pink-400 border-r border-gray-800">Lab</td>
 ...
 <div className="flex gap-4 mt-4 px-2 text-xs font-mono text-gray-400">
 <div className="flex items-center gap-1"><div className="w-3 h-3 bg-transparent border border-gray-700"></div> Empty</div>
