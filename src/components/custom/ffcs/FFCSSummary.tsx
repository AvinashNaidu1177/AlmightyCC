    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-center">
                    <div className="text-3xl font-bold text-purple-600 dark:text-purple-400 font-mono">{courses.length}</div>
                    <div className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-wider">Courses</div>
                </div>
                <div className={`p-4 rounded-lg border text-center ${totalCr > 30 ? "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800" : totalCr > 25 ? "bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800" : "border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900"}`}>
                    <div className={`text-3xl font-bold font-mono ${totalCr > 30 ? "text-red-600 dark:text-red-400" : totalCr > 25 ? "text-orange-600 dark:text-orange-400" : "text-purple-600 dark:text-purple-400"}`}>{totalCr}</div>
                    <div className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-wider">Credits</div>
                </div>
                <div className={`p-4 rounded-lg border text-center ${nConflict > 0 ? "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800" : "border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900"}`}>
                    <div className={`text-3xl font-bold font-mono ${nConflict > 0 ? "text-red-600 dark:text-red-400" : "text-purple-600 dark:text-purple-400"}`}>{nConflict}</div>
                    <div className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-wider">Conflicts</div>
                </div>
                <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-center">
                    <div className="text-3xl font-bold text-pink-600 dark:text-pink-400 font-mono">{nLabs}</div>
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800">
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 dark:bg-gray-900 text-purple-600 dark:text-purple-400 font-mono text-xs uppercase">
                        <tr>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-[#111827]">
                        {calculatedCourses.map((c, i) => (
                            <tr key={c.id} className={c.hasConflict ? "bg-red-50 dark:bg-red-900/10 border-l-4 border-l-red-500" : "hover:bg-gray-50 dark:hover:bg-gray-800/50"}>
                                <td className="px-4 py-3 font-mono text-gray-500">{i + 1}</td>
                                <td className="px-4 py-3 font-bold font-mono text-purple-600 dark:text-purple-400">{c.code}</td>
                                <td className="px-4 py-3">{c.name || "—"}</td>
                                <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{c.faculty || "—"}</td>
                                <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{c.room || "—"}</td>
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-1 rounded-full text-xs font-mono border ${c.hasConflict ? "bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800" : "bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800"}`}>
                                        {c.slots.join("+")}
                                    </span>
                                </td>
                    ...
                    <tfoot className="bg-gray-50 dark:bg-gray-900 font-bold border-t-2 border-gray-200 dark:border-gray-800">
                        <tr>
                            <td colSpan={6} className="px-4 py-3 text-right text-xs font-mono text-gray-500">TOTAL CREDITS</td>
                            <td className={`px-4 py-3 text-right font-mono ${totalCr > 30 ? "text-red-600 dark:text-red-400" : "text-purple-600 dark:text-purple-400"}`}>{totalCr}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div className="space-y-2">
                ...
                {totalCr > 25 && totalCr <= 30 && (
                    <div className="flex items-center gap-3 p-4 rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 text-orange-700 dark:text-orange-400 text-sm">
